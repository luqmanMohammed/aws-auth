    ...
    match subcommand {
        ...
        Alias::List {
            ...
            let aliases: Vec<[&str; 3]> = alias_provider
                .list_aliases()
                .map_err(Error::AliasProvider)?;
            let omit_fields = omit_fields.iter().map(|v| v.as_str()).collect();

            match output {
                crate::cmd::OutputFormat::Json => {
                    let formatter = JsonFormatter::new(omit_fields, no_headers);
                ...
                crate::cmd::OutputFormat::Text => {
                    let formatter = TextFormatter::new(omit_fields, no_headers, " | ");
                    let output = formatter
